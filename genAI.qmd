...
```{=html}
<div id="model-cost-exercise" style="font-family: 'Georgia', serif; max-width: 860px; margin: 1.5rem 0; border: 1px solid #d1d5db; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.07);">

  <!-- Header -->
  <div style="background: #1e3a5f; color: white; padding: 1rem 1.4rem;">
    <div style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.7; margin-bottom: 0.25rem;">Interactive Exercise</div>
    <div style="font-size: 1.05rem; font-weight: 600;">Balancing model performance and cost</div>
  </div>

  <!-- Body -->
  <div style="padding: 1.2rem 1.4rem; background: #f9fafb;">
    <p style="margin: 0 0 1rem; font-size: 0.92rem; color: #374151; line-height: 1.6;">
      Your AI workloads range from basic code autocompletion to complex enterprise refactoring. Faster, cheaper models handle simpler tasks well — but complex reasoning requires more capable ones. Classify each task into the correct bucket.
    </p>

    <!-- Drop zone -->
    <div style="margin-bottom: 1.2rem;">
      <div style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.07em; color: #6b7280; margin-bottom: 0.5rem;">Drag items to sort:</div>
      <div id="mc-source-zone"
        ondragover="event.preventDefault()"
        ondrop="mcDropToSource(event)"
        style="min-height: 64px; background: #1e3a5f; border-radius: 8px; padding: 0.75rem; display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: flex-start;">
      </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
      ...
      <div>
        <div style="font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: #166534; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 0.4rem;">
          <span style="display:inline-block; width:10px; height:10px; background:#16a34a; border-radius:50%;"></span> Can use regular (cheaper) models
        </div>
        <div id="mc-regular-zone"
          ondragover="event.preventDefault()"
          ondrop="mcDropTo(event, 'regular')"
          style="min-height: 140px; background: #f0fdf4; border: 2px dashed #86efac; border-radius: 8px; padding: 0.6rem; display: flex; flex-direction: column; gap: 0.5rem;">
          <div style="color:#86efac; font-size:0.8rem; text-align:center; margin:auto; pointer-events:none;" class="mc-placeholder">Drop here</div>
        </div>
      </div>

      <!-- Requires powerful -->
      <div>
        <div style="font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: #92400e; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 0.4rem;">
          <span style="display:inline-block; width:10px; height:10px; background:#d97706; border-radius:50%;"></span> Requires powerful models
        </div>
        <div id="mc-powerful-zone"
          ondragover="event.preventDefault()"
          ondrop="mcDropTo(event, 'powerful')"
          style="min-height: 140px; background: #fffbeb; border: 2px dashed #fcd34d; border-radius: 8px; padding: 0.6rem; display: flex; flex-direction: column; gap: 0.5rem;">
          <div style="color:#fcd34d; font-size:0.8rem; text-align:center; margin:auto; pointer-events:none;" class="mc-placeholder">Drop here</div>
        </div>
      </div>
    </div>
    ...
    <!-- Buttons -->
    <div style="margin-top: 1rem; display: flex; gap: 0.6rem; flex-wrap: wrap;">
      <button onclick="mcCheckAnswers()"
        style="background:#1e3a5f; color:white; border:none; padding: 0.45rem 1.1rem; border-radius:6px; font-size:0.85rem; cursor:pointer; font-family:inherit;">
        Check Answers
      </button>
      <button onclick="mcReset()"
        style="background:white; color:#374151; border:1px solid #d1d5db; padding: 0.45rem 1.1rem; border-radius:6px; font-size:0.85rem; cursor:pointer; font-family:inherit;">
        Reset
      </button>
    </div>
    ...
    <!-- Feedback -->
    <div id="mc-feedback" style="margin-top: 0.8rem; font-size: 0.88rem; display:none;"></div>
  </div>
</div>
...
(function() {
  const mcItems = [
    { id: "mc1", text: "Adding docstrings to existing functions.",                          answer: "regular"  },
    { id: "mc2", text: "Generating basic unit test templates.",                             answer: "regular"  },
    { id: "mc3", text: "Autocomplete for regular Python functions.",                        answer: "regular"  },
    { id: "mc4", text: "Translating a simple SQL query to pandas.",                         answer: "regular"  },
    { id: "mc5", text: "Refactoring a legacy module across multiple interdependent files.", answer: "powerful" },
    { id: "mc6", text: "Fixing a complex bug that spans multiple files.",                   answer: "powerful" },
    { id: "mc7", text: "Explaining the time complexity of a recursive algorithm.",          answer: "powerful" },
    { id: "mc8", text: "Designing a secure authentication flow with edge-case handling.",   answer: "powerful" },
  ];
  ...
  function mcClearPlaceholders(zone) {
    zone.querySelectorAll(".mc-placeholder").forEach(el => el.remove());
  }

  window.mcDropTo = function(event, target) {
    event.preventDefault();
    const id = event.dataTransfer.getData("text/plain");
    const tile = document.getElementById(id);
    if (!tile) return;
    const zone = document.getElementById("mc-" + target + "-zone");
    mcClearPlaceholders(zone);
    zone.appendChild(tile);
    tile.style.border = "1px solid #d1d5db";
    tile.style.background = "white";
    tile.style.color = "#1f2937";
  ...
  window.mcDropToSource = function(event) {
    event.preventDefault();
    const id = event.dataTransfer.getData("text/plain");
    const tile = document.getElementById(id);
    if (!tile) return;
    document.getElementById("mc-source-zone").appendChild(tile);
    tile.style.border = "1px solid #d1d5db";
    tile.style.background = "white";
    tile.style.color = "#1f2937";
    document.getElementById("mc-feedback").style.display = "none";
  ...
  window.mcCheckAnswers = function() {
    const regular = document.getElementById("mc-regular-zone");
    const powerful = document.getElementById("mc-powerful-zone");
    let correct = 0, total = 0;

    [...regular.children, ...powerful.children].forEach(tile => {
      if (!tile.dataset.answer) return;
      total++;
      const zone = tile.parentElement.id.replace("mc-", "").replace("-zone", "");
      if (zone === tile.dataset.answer) {
        tile.style.border = "2px solid #16a34a";
        tile.style.background = "#f0fdf4";
        correct++;
    ...
    if (total === 0) {
      fb.innerHTML = `<span style="color:#6b7280;">Sort all items before checking.</span>`;
    } else if (correct === mcItems.length) {
      fb.innerHTML = `<span style="color:#166534; font-weight:600;">✓ All correct!</span> Routine, pattern-based tasks (docstrings, basic tests, autocomplete) are well-handled by lighter models. Tasks requiring multi-step reasoning, cross-file context, or security judgment need frontier-tier capability.`;
    } else {
      fb.innerHTML = `<span style="color:#991b1b; font-weight:600;">${correct} of ${mcItems.length} correct.</span> Red items are in the wrong bucket — consider whether the task requires multi-step reasoning or just pattern completion.`;
    }
  };

  window.mcReset = function() {
    const src = document.getElementById("mc-source-zone");
    src.innerHTML = "";
    document.getElementById("mc-regular-zone").innerHTML  = `<div style="color:#86efac; font-size:0.8rem; text-align:center; margin:auto; pointer-events:none;" class="mc-placeholder">Drop here</div>`;
    document.getElementById("mc-powerful-zone").innerHTML = `<div style="color:#fcd34d; font-size:0.8rem; text-align:center; margin:auto; pointer-events:none;" class="mc-placeholder">Drop here</div>`;
    document.getElementById("mc-feedback").style.display = "none";
    mcShuffle(mcItems).forEach(item => src.appendChild(mcMakeTile(item)));
  };

  // Init on load
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", mcReset);
  } else {
    mcReset();
  }
})();
